...
const Inventory = () => {
  ...
  useEffect(() => {
    // On écoute la collection 'inventory'
  ...
  return (
    <div className="inventory-container">
      <h2>Gestion du Stock Magasin</h2>
      ...
      <div className="table-responsive">
        <table className="inventory-table">
          <thead>
            <tr>
              <th>Article</th>
              <th>Prix d'Achat (F)</th> {/* Changé en Prix d'Achat */}
              <th>Stock</th>
              <th>Actions</th>
            </tr>
          </thead>
            ...
            {inventory.map(item => {
              ...
              return (
                <tr key={item.id} className={isLowStock ? 'low-stock' : ''}>
                  ...
                  {/* Affichage du prix d'achat récupéré de Firebase */}
      ...
      {inventory.length === 0 && (
        <p className="empty-msg">Aucun article en stock. Vérifiez la collection "inventory".</p>
      )}
    </div>
  );
};
